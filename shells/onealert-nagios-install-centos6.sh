...
if [ -d /etc/$USER_NAME/ ];then
    $sudo_cmd rm -rf /etc/$USER_NAME/
fi
...
$sudo_cmd /usr/bin/onealert-config --init --token $AppKey
echo -e "initialize onealert done."
echo -e "start add $Plugin plugin."
$sudo_cmd /usr/bin/onealert-config --add $Plugin --app-key $AppKey
